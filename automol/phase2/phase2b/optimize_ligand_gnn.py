...
def optimize_ligand(initial_smiles: str, protein_sequence: str, model: EnhancedTelomeraseGNN, num_iterations: int = 10) -> str:
    ...
    for i in range(num_iterations):
        ...
        try:
            ...
            if new_smiles is None or new_smiles == current_smiles:
                logging.info(f"MCTS did not find a valid modification in iteration {i+1}")
                continue
    ...
    logging.info(f"Optimization complete. Final SMILES: {best_smiles}, Best score: {best_score}")
    return best_smiles




# Update the optimize_ligand_smiles function to use this new load_model function
def optimize_ligand_smiles(initial_smiles, protein_sequence):
    ...
    try:
        ...
        logger.error(f"Error during ligand optimization: {str(e)}")
        return initial_smiles  # Return the initial SMILES if optimization fails
